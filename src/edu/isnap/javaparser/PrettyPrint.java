...
public class PrettyPrint {
	...
	public static String toString(ASTNode node, Params params) {
		return print(node, "", params);
	}

	private  static String print(ASTNode node, String indent, Params params) {
		if (node == null) return "";
		String out = params.baseString(node);
		boolean inline = !params.isBodyType.test(node.type);
		List<? extends ASTNode> children = node.children;
		if (children.size() > 0) {
			if (inline) {
				out += "(";
	...
	public static class Params {
		...
		public String baseString(ASTNode node) {
